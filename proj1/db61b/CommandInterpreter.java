...
class CommandInterpreter {
    ...
    Table selectClause(String tableName) {
        ...
        while (!_input.nextIf("from")) {
            if (_input.nextIs(";")) {
                throw error("No table to select from.");
            }
            Column selected = columnSelector();
            selectColumns.add(selected);
            if (_input.nextIf("as")) {
                String name = name();
                columnTitles.add(name);
            } else {
                columnTitles.add(selected.name());
            }
            while (_input.nextIf(",")) {
        ...
        iterators.add(tableName().tableIterator()); // Change for two or more tables
        Table table = new Table(tableName, columnTitles);
        List<Condition> conditions = new ArrayList<Condition>();
        if (_input.nextIs("where")) {
            conditions = conditionClause(iterators);
        }
        select(table, selectColumns, iterators, conditions);
        return table; // TEST
    }
    ...
    Column columnSelector() {
        Column newColumn = new Column(null, "name");
        String temp = _input.peek();
        try {
            Table table = tableName();
            _input.next(".");
            String columnName = name();
            newColumn = new Column(table, columnName);
        } catch (DBException e) {
            newColumn = new Column(null, temp);
        }
        return newColumn;
    }
    ...
    Condition condition(List<TableIterator> iterators) {
        ...
        if (_input.nextIs(Tokenizer.LITERAL)) {
            ...
            Condition cond = new Condition(col1, relation, literal);
            return cond;
        } else {
            Column col2 = columnSelector(iterators);
            Condition cond = new Condition(col1, relation, col2);
            return cond;
        } // TEST
    }
    ...
    private void select(Table table, ArrayList<Column> columns,
        ...
        if (iterators.size() > 2) {
            throw error("Cannot select from more than two tables.");
        }
        for (Column column : columns) {
            column.resolve(iterators);
        }
        Row row = new Row(columns); // ADD HANDLING FOR TWO TABLES
        table.add(row);
        while (iterators.get(0).next() != null) {
            row = new Row(columns);
            table.add(row);
